    ...
    for node in old_nodes:
        ...
        if not images:
            ...
            for image_elements in images:
                ...
                if text_chunks[0]:
                    new_nodes.append(TextNode(text_chunks[0], TEXT_TYPE_TEXT))
                new_nodes.append(
                    TextNode(image_elements[0], TEXT_TYPE_IMAGE, url=image_elements[1])
                )
                text = text_chunks[1]
            if text:
    ...
    for node in old_nodes:
        ...
        if not links:
            ...
            for link_elements in links:
                ...
                if text_chunks[0]:
                    new_nodes.append(TextNode(text_chunks[0], TEXT_TYPE_TEXT))
                new_nodes.append(TextNode(link_elements[0], TEXT_TYPE_LINK, url=link_elements[1]))
                text = text_chunks[1]
            if text:
                new_nodes.append(TextNode(text, TEXT_TYPE_TEXT))
    return new_nodes
